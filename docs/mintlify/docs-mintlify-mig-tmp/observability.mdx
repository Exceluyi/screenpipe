...
screenpipe can mirror operational telemetry from the CLI, SDK, and embedded recorder runtimes to backends that you control. This is opt-in and configured with environment variables; when none of these variables are set, behavior is unchanged.
...
- SDK recorder start/stop/error events for apps that embed `@screenpipe/sdk` or the Rust recorder crate
- API usage counters
